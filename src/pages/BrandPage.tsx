              ...
              <Reveal key={p.name} delay={(i % 2) * 0.1}>
                <div className={`grid items-center gap-10 lg:grid-cols-12 lg:gap-16 ${i % 2 ? "lg:[direction:rtl]" : ""}`}>
                  <div className="lg:col-span-7 lg:[direction:ltr]">
                    <div className="relative aspect-[4/3] overflow-hidden rounded-md" style={{ background: `linear-gradient(135deg, ${brand.accent}33, ${brand.accent}11)` }}>
      ...
      <section className="bg-bone py-24">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {BRANDS.filter((b) => b.slug !== brand.slug).map((otherBrand) => (
              <Link
                key={otherBrand.slug}
                to={`/brands/${otherBrand.slug}`}
                className="group block overflow-hidden rounded-md bg-white ring-1 ring-black/5 transition-all hover:-translate-y-1 hover:shadow-xl"
              >
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={otherBrand.image} alt={otherBrand.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                </div>
                <div className="p-5">
                  <p className="font-display text-2xl">{otherBrand.name}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{otherBrand.category}</p>
                </div>
              </Link>
            ))}
          </div>
